...
function domainsPartial(appName: string, domains: string[]) {
  return html`
    <!-- Add domain form -->
              ...
              ${domains.map(
                (domain) => html`
                  <div class="flex items-center justify-between px-4 py-2.5 group hover:bg-gray-50">
                    ...
                    <form method="POST" action="/apps/${appName}/domains/remove" class="opacity-0 group-hover:opacity-100 transition-opacity">
...
export function domainsRoutes() {
  ...
  app.get("/", async (c) => {
    ...
    try {
      ...
      const content = domainsPartial(name, domains);

      if (partial === "1") return c.html(content);
      return c.html(layout(name, appDetailPage(name, "domains", content, appInfo), "/apps", c.get("userEmail")));
    } catch (err) {
      const message = err instanceof Error ? err.message : "Failed to get domains";
      const content = alert("error", message);
      if (partial === "1") return c.html(content);
      return c.html(layout(name, appDetailPage(name, "domains", content), "/apps", c.get("userEmail")));
    }
  });
